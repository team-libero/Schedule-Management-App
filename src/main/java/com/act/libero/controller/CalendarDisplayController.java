...
public class CalendarDisplayController {
	 ...
	 * スケジュール情報:DBから取得後セッションに格納（月単位）
	 */
	@GetMapping(value = "/calendarDisplay/select")
	public String calendarDisplaySelect(@ModelAttribute ScheduleInfo scheduleInfo, Model model,
			@RequestParam(required = false) String targetDate) {
		// String selectDate, HttpSession sessionInfo) {
		...
		// 対象日付をセット
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		String searchDate = sdf.format(targetDate);
	 ...
	 * スケジュール情報:DBから取得後セッションに格納（月単位）
	 */
	@GetMapping(value = "/calendarDisplay/selectDay")
